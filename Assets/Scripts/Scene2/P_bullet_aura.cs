...
public class P_bullet_aura : MonoBehaviour
{
    ...
    void Update()
    {
        if(gameObject.GetComponent<Renderer>().bounds.Intersects(player.GetComponent<Renderer>().bounds) && currentDamageTime>=damageInterval && !scenemanager.GetComponent<Scene2_Manager>().s2.activeSelf){
